...
function toggleVariantRow(rowId: string) {
  ...
  return <>
    ...
    {rowMenu ? <div ref={rowMenuRef} className="product-grid-row-menu" role="menu" aria-label="เมนูสินค้า" style={{ left: rowMenu.left, top: rowMenu.top }} onKeyDown={(event) => {
      ...
      <Link role="menuitem" href={detailHref({ organizationId, search, status, dateField, dateFrom, dateTo, brandId, categoryId, tagIds, priceMin, priceMax, stockMin, stockMax, sort, productId: rowMenu.rowId, skuId: rowMenu.skuId, page: currentPage, pageSize, bulkSearchActive, action: 'images' })}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="16" rx="2" /><circle cx="9" cy="10" r="2" /><path d="m5 18 5-5 3 3 2-2 4 4" /></svg><span className="product-grid-row-menu-copy"><strong>จัดการรูปภาพ</strong><small>เพิ่ม เปลี่ยนภาพปก และจัดลำดับ</small></span></Link>
      <Link role="menuitem" href={detailHref({ organizationId, search, status, dateField, dateFrom, dateTo, brandId, categoryId, tagIds, priceMin, priceMax, stockMin, stockMax, sort, productId: rowMenu.rowId, skuId: rowMenu.skuId, page: currentPage, pageSize, bulkSearchActive, action: 'edit' })}><IconEdit aria-hidden="true" /><span className="product-grid-row-menu-copy"><strong>แก้ไขข้อมูลสินค้า</strong><small>ชื่อ หมวดหมู่ แบรนด์ Tags และข้อมูลส่วนกลาง</small></span></Link>
