...
import { MasterDataManager, type ProductMasterOption } from './new/unified-product-creation-form'

type MasterOption = ProductMasterOption
...
export function ProductsBulkEdit(props: Props) {
  ...
  const [brandMasterOptions, setBrandMasterOptions] = useState<ProductMasterOption[]>(props.brandOptions)
  const [categoryMasterOptions, setCategoryMasterOptions] = useState<ProductMasterOption[]>(props.categoryOptions)
  const [tagMasterOptions, setTagMasterOptions] = useState<ProductMasterOption[]>(props.tagOptions)
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && !pending && !document.querySelector('.product-master-dialog')) setAction(null)
    }
  ...
  function productBody() {
    ...
    const options = action === 'brand' ? [{ id: '', name: 'ไม่มีแบรนด์' }, ...brandMasterOptions.filter((option) => option.status !== 'archived')] : action === 'category' ? categoryMasterOptions.filter((option) => option.status !== 'archived') : tagMasterOptions.filter((option) => option.status !== 'archived')
