...
type PreviewState = {
  ...
  existing: Set<string>
  grandfatheredSalesCodes: Set<string>
  blankSalesCodeCount: number
  proposedSalesCodeRange: { startCode: string; endCode: string; quantity: number } | null
  commandIds: Record<number, string>
  batchCommandIds: string[]
}
...
export function ProductExcelImportDialogLive({ open, organizationId, onClose }: { open: boolean; organizationId: string; onClose: () => void }) {
  ...
  async function inspectFile() {
    ...
    try {
      ...
      const salesCodes = validation.rows.map((row) => row.salesCode).filter((value): value is string => Boolean(value))
      const blankSalesCodeCount = validation.rows.filter((row) => !row.salesCode).length
      const check = identifiers.length || blankSalesCodeCount ? await checkProductImportIdentifiersAction({
        organizationId, identifiers, salesCodes, blankSalesCodeCount,
      }) : { ok: true as const, data: { checked: 0, existing: [], conflictingSalesCodes: [], grandfatheredSalesCodes: [], blankSalesCodeCount: 0, proposedSalesCodeRange: null } }
      ...
      setPreview({
        ...
        grandfatheredSalesCodes: new Set(check.data.grandfatheredSalesCodes),
        blankSalesCodeCount: check.data.blankSalesCodeCount,
        proposedSalesCodeRange: check.data.proposedSalesCodeRange,
        commandIds: Object.fromEntries(validation.rows.map((row) => [row.sourceRow, crypto.randomUUID()])),
        batchCommandIds: Array.from({ length: Math.ceil(validation.rows.length / 50) }, () => crypto.randomUUID()),
      })
      setStep('preview')
    } catch (caught) {
  ...
  async function importRows() {
    ...
    try {
      for (let index = 0; index < readyRows.length; index += 50) {
        const batch = readyRows.slice(index, index + 50).map((row) => ({ ...row, commandId: preview.commandIds[row.sourceRow] }))
        const response = await executeProductImportRowsAction({
          organizationId,
          batchCommandId: preview.batchCommandIds[Math.floor(index / 50)],
          rows: batch,
        })
        if (!response.ok) {
      ...
      setError('การนำเข้าหยุดก่อนเสร็จ ชุดที่ไม่สำเร็จถูกย้อนกลับทั้งหมด คุณสามารถกดลองใหม่โดยระบบจะใช้ Command ID เดิมและไม่สร้างรายการซ้ำ')
      setStep('complete')
    } finally { setBusy(false) }
  }
  ...
  return <div className="product-modal-backdrop product-excel-import-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <section ref={dialogRef} className="product-excel-import-dialog" role="dialog" aria-modal="true" aria-labelledby="product-excel-import-title" aria-describedby="product-excel-import-description" onKeyDown={handleKeyDown}>
      ...
      <div className="product-excel-import-body">
        ...
        {step === 'importing' ? <div className="product-excel-import-complete" role="status" aria-live="polite"><IconUpload aria-hidden="true" size={42} /><h3>กำลังนำเข้าสินค้า</h3><p>ดำเนินการครั้งละไม่เกิน 50 รายการแบบ All-or-Nothing และไม่เปลี่ยน Stock</p><progress max={Math.max(totalToImport, 1)} value={processed} /><strong>{processed.toLocaleString('th-TH')} / {totalToImport.toLocaleString('th-TH')} รายการ</strong></div> : null}
...
function Preview({ file, preview, issueByRow, readyCount, existingCount, errorCount }: { file: File | null; preview: PreviewState; issueByRow: Map<number, ProductImportIssue[]>; readyCount: number; existingCount: number; errorCount: number }) {
  return <><div className="product-excel-import-preview-banner"><IconFileSpreadsheet aria-hidden="true" size={22} /><div><strong>{file?.name}</strong><span>อ่านข้อมูลจริง {preview.rows.length.toLocaleString('th-TH')} แถว · ยังไม่บันทึกฐานข้อมูล</span>{preview.proposedSalesCodeRange ? <small>Sales Code ว่าง {preview.blankSalesCodeCount.toLocaleString('th-TH')} รายการ · ชุดแรกเสนอ {preview.proposedSalesCodeRange.startCode}–{preview.proposedSalesCodeRange.endCode}</small> : null}</div></div><div className="product-excel-import-summary" aria-label="สรุปผลตรวจสอบ"><div><span>ทั้งหมด</span><strong>{preview.rows.length}</strong></div><div data-tone="success"><span>พร้อมนำเข้า</span><strong>{readyCount}</strong></div><div data-tone="warning"><span>รหัสขัดแย้ง</span><strong>{existingCount}</strong></div><div data-tone="danger"><span>ต้องแก้ไข</span><strong>{errorCount}</strong></div></div><section className="product-excel-import-preview-table" aria-labelledby="product-excel-import-preview-title"><div className="product-excel-import-preview-heading"><div><strong id="product-excel-import-preview-title">ตัวอย่างรายการก่อนยืนยัน</strong><span>แสดงข้อมูลจริงสูงสุด 100 แถวแรก</span></div><span className="product-excel-import-policy-chip">ต้องแก้รหัสซ้ำก่อนนำเข้า</span></div><div className="product-excel-import-table-scroll"><table><thead><tr><th>แถว</th><th>SKU</th><th>Sales Code</th><th>สินค้า</th><th>ผลตรวจสอบ</th></tr></thead><tbody>{preview.rows.slice(0, 100).map((row) => { const issues = issueByRow.get(row.sourceRow) ?? []; const exists = rowIdentifiers(row).some((value) => preview.existing.has(value)); const grandfathered = Boolean(row.salesCode && preview.grandfatheredSalesCodes.has(row.salesCode)); const result = issues.length ? issues[0].message : grandfathered ? 'รหัสเก่าอ่านและค้นหาได้ แต่ห้ามนำไปสร้างรายการใหม่' : exists ? 'รหัสซ้ำใน Organization' : !row.salesCode ? 'ระบบจะสร้าง Sales Code อัตโนมัติ' : 'พร้อมเพิ่ม'; const tone = issues.length || exists ? 'danger' : !row.salesCode ? 'warning' : 'success'; return <tr key={row.sourceRow}><td>{row.sourceRow}</td><td>{row.skuCode}</td><td>{row.salesCode ?? 'อัตโนมัติ'}</td><td>{row.productName}</td><td><span className="product-excel-import-result" data-tone={tone}>{result}</span></td></tr> })}</tbody></table></div></section></>
}
