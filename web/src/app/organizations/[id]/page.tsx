...
export default async function OrganizationPage({ params }: Props) {
  ...
  return <main className="dashboard"><header className="topbar"><div className="brand">AVENZO ONE / {organization.name}</div><div className="topbar-actions"><span>{user.email}</span><SignOutButton /></div></header><section className="content"><div className="hero"><div><div className="eyebrow">Workspace</div><h1>{organization.name}</h1><p>/{organization.slug} · {organization.timezone} · {organization.currency}</p></div><a className="button secondary" href="/dashboard">กลับ Dashboard</a></div><div className="grid"><article className="card"><h2>Branches</h2><p>สาขาภายใน Organization</p>{branches?.length ? <div>{branches.map((branch) => <div className="meta" key={branch.id}>{branch.code} · {branch.name} · {branch.status}</div>)}</div> : <div className="empty">ยังไม่มี Branch</div>}<CreateBranchForm organizationId={id} /></article><article className="card"><h2>เชิญสมาชิก</h2><p>สร้างคำเชิญพร้อม Role และ Branch Scope</p><InviteMemberForm organizationId={id} roles={roles ?? []} branches={branches ?? []} /></article></div><div className="card" style={{ marginTop: 18 }}><h2>สมาชิก</h2>{members?.length ? <div className="table">{members.map((member) => <div className="table-row" key={member.id}><span>{member.user_id}</span><span>{member.membership_status}</span><span>{member.scope}</span></div>)}</div> : <div className="empty">ยังไม่มีสมาชิก</div>}</div><div className="card" style={{ marginTop: 18 }}><h2>คำเชิญล่าสุด</h2>{invitations?.length ? <div className="table">{invitations.map((invitation) => <div className="table-row invitation-row" key={invitation.id}><span>{invitation.email}</span><span>{invitation.role_code}</span><span>{invitation.status}</span>{invitation.status === 'pending' ? <CancelInvitationButton invitationId={invitation.id} /> : <a className="button secondary" href={`/invitations/${invitation.id}`}>เปิดคำเชิญ</a>}</div>)}</div> : <div className="empty">ยังไม่มีคำเชิญ</div>}</div></section></main>
}
