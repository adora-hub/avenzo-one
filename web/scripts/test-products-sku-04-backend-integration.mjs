...
test('SKU-04 adds a service-only organization and Prefix sequence boundary', async () => {
  const [sql, corrective] = await Promise.all([
    read('../supabase/migrations/20260820134813_phase_2_1_sku_04_product_sequence_allocator.sql'),
    read('../supabase/migrations/20260821060222_sku_04_granular_product_create_authority.sql'),
  ])
  ...
  assert.match(corrective, /server_preview_variant_sku_sequence[\s\S]+product\.create/)
  assert.match(corrective, /p_command_type = 'product\.variant_images\.assign'[\s\S]+product\.update/)
  assert.match(corrective, /Authorize before replay lookup[\s\S]+foundation_commands/)
  assert.doesNotMatch(corrective, /server_actor_has_org_permission\([\s\S]{0,200}'product\.manage'/)
  assert.doesNotMatch(corrective, /create table|create policy|create trigger|insert into public\.permissions/i)
})
...
test('SKU-04 UI previews the database next sequence without reserving it', async () => {
  ...
  assert.match(builder, /ระบบแนะนำเลขว่างถัดไปจากฐานข้อมูล/)
