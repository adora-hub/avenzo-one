...
test('GSC-05 serializes one global command ID before insert without weakening constraints', () => {
  const permissionCheck = migration.indexOf("private.server_actor_has_org_permission(\n    p_actor_user_id, p_organization_id, 'product.create', null")
  ...
  const commandInsert = migration.indexOf('insert into public.global_sales_code_creation_commands')

  assert.ok(permissionCheck >= 0 && permissionCheck < advisoryLock)
  assert.ok(advisoryLock < commandInsert)
  assert.match(migration, /on conflict \(id\) do nothing/)
  assert.match(migration, /where c\.id = p_command_id\s+for update/)
  assert.doesNotMatch(migration, /hashtextextended\([^)]*organization_id/i)
  assert.doesNotMatch(migration, /alter table public\.global_sales_code_creation_commands[\s\S]{0,120}drop constraint/i)
  assert.doesNotMatch(migration, /drop index[^;]*global_sales_code_creation_commands/i)
})
