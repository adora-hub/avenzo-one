...
วันที่: 20–21 สิงหาคม 2026
สถานะ: **Approved/Closed — Base and T5.3 Corrective Migrations Applied to AVENZO ONE PREVIEW — Production Untouched**
...
- Apply Forward-only Corrective Migration `20260821060222` บน PREVIEW แล้ว เพื่อเปลี่ยน authority จาก legacy `product.manage` เป็น `product.create`/`product.update` ตาม T4.3B
- Corrective path ใช้ effective-permission helper เดิม จึงไม่มี permission alias ที่ข้าม Individual Deny และไม่มี Browser grant เพิ่ม
...
- T5.3 PREVIEW P01–P14, Security และ Lint: PASS; Advisor มีเฉพาะรายการ INFO ที่ไม่ใช่ Contract failure
