...
### Pre-release compatibility correction — 22 August 2026

Before any PREVIEW or Production apply, integrated replay against the T4.3B
permission contract found that the idempotent `product.create` catalog upsert
omitted the required `permissions.scope_kind`. GSC-04 now inserts and reconciles
that code with `scope_kind = 'organization'`. This correction changes no
permission grant, RLS policy, default, trigger or nullable constraint. GSC-04
has never been applied to PREVIEW or Production.
...
- `product.create` is the granular permission for allocation. The migration
  ...
  `product.manage`, and reconciles its catalog scope as `organization`.
