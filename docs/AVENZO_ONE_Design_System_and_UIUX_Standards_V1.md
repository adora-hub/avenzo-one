...
- **Primary Menu Button:** Action ที่เปิดตัวเลือกย่อยและมีความสำคัญระดับเดียวกับปุ่มหลัก เช่น `สร้างสินค้า` หรือ `บันทึกร่าง` ใช้ปุ่มพื้นดำ ตัวอักษรขาว Radius 10px ความสูง 40–44px พร้อมลูกศรลงอยู่ต่อท้ายข้อความภายในปุ่มเดียว; Hover/Focus/Active ใช้พื้นเทาเข้ม และรายการในเมนูต้องมีชื่อ Action กับคำอธิบายผลลัพธ์สั้น ๆ ห้ามแยกข้อความกับลูกศรเป็นปุ่มคนละชิ้น
- **Back Navigation Button:** ปุ่มกลับไปหน้ารายการใช้ `secondary` พื้น Surface เส้นขอบมาตรฐาน พร้อมไอคอนลูกศรซ้ายขนาด 16px อยู่หน้าข้อความ เช่น `กลับหน้าสินค้า`; ต้องใช้คำบอกปลายทาง ห้ามใช้คำกว้าง ๆ ว่า `ยกเลิก` เมื่อการกดเพียงนำทางออกจากหน้าและยังเก็บ Draft ไว้
...
- งานแบบ Batch ต้องแสดงผลสำเร็จเพียงหนึ่งตำแหน่ง ไม่ซ้ำทั้ง Validation และ Execution state; ข้อความต้องสรุปจำนวนรายการและผลที่เกิดขึ้นจริง พร้อมปิดเองได้และมีปุ่มปิด โดยสถานะถาวรยังคงแสดงในแต่ละแถว
- Action ล้าง Draft หรือยกเลิกงานชุดต้องระบุผลกระทบแยก `ข้อมูลใน Browser`, `สินค้าที่สร้างแล้ว`, `สต็อกที่รับแล้ว` และ `รหัสที่ยังจอง` ก่อนยืนยัน ห้ามใช้ข้อความที่ทำให้เข้าใจว่าเป็นการลบหรือ Rollback ข้อมูลจริงหากระบบทำเพียง Local cleanup
- Badge จำนวนที่ใช้บอกขอบเขตการแก้ไขต้องใช้พื้นดำ ตัวอักษรขาว เพื่อแยกจาก Accent สีเขียวซึ่งสงวนไว้สำหรับ Primary action และ Success state
- ปุ่มที่เปิดขั้นตรวจและส่งสร้างต้องเป็น Primary action ที่เด่นที่สุดของ Section ใช้คำกริยาบอกผลลัพธ์ เช่น `ตรวจและสร้างรายการ` และมีความสูงไม่น้อยกว่า 40px
- ปุ่มที่อยู่ใน Action Group แถวเดียวกันต้องมีความสูงเท่ากัน แม้เป็น Primary และ Secondary; แยกลำดับความสำคัญด้วยสี น้ำหนักตัวอักษร และ Padding แนวนอน ไม่ใช้ความสูงต่างระดับ
- หน้าสำหรับผู้ใช้งานจริงห้ามแสดงคำเชิงพัฒนา เช่น `UI Simulation`, `Local Backend`, `PREVIEW` หรือชื่อเทคนิคของ Transaction; ให้สื่อสารสิ่งที่ผู้ใช้ทำได้และผลที่จะเกิดขึ้นแทน

### Floating Back-to-top

- หน้าทำงานที่มีเนื้อหายาวควรมีปุ่มลอย `กลับด้านบน` ติดมุมขวาล่างของ Viewport เพื่อให้เข้าถึงได้โดยไม่ต้องเลื่อนหาปุ่ม
- ใช้ทรงกลมขนาด 42px, ไอคอนลูกศรขึ้นขนาดประมาณ 18–20px, พื้นหลัง Surface, เส้นขอบมาตรฐาน และเงาระดับ Floating action
- ต้องมี `aria-label` และ `title` ว่า `กลับด้านบน`, รองรับ Keyboard และใช้ Anchor ไปยังจุดเริ่มต้นของเนื้อหาเพื่อไม่เพิ่ม Client state โดยไม่จำเป็น
- Hover ใช้พื้นหลัง `surface-subtle` และเส้นขอบเข้มขึ้นเล็กน้อย; ห้ามใช้สี Primary แข่งกับปุ่มงานหลักของหน้า
- หน้า Product Creation และ Live Sale / Rapid Entry ต้องใช้ Pattern และ Class กลาง `product-back-to-top` เดียวกัน ห้ามใช้ตัวอักษรลูกศรหรือสร้างปุ่มเฉพาะหน้าที่มีขนาด/สีต่างกัน
...
- ชื่อสถานะต้องไม่ซ้อนความหมายกัน: ใช้ `รอดำเนินการ` สำหรับรายการที่ยังว่างรวมกับรายการที่ข้อมูลไม่ครบ และใช้ `ข้อมูลไม่ครบ` สำหรับรายการที่เริ่มกรอกแล้วแต่ยังไม่ครบหรือไม่ถูกต้อง
