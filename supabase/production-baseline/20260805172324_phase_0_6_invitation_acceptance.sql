...
returns table (invitation_id uuid, organization_id uuid, organization_name text, membership_id uuid)
language plpgsql
security definer
...
begin
  ...
  where i.id = p_invitation_id and lower(i.email) = v_email
  for update;
  ...
  if not exists (select 1 from public.organizations o where o.id = v_invitation.organization_id and o.status = 'active') then
    raise exception 'organization_not_active';
  end if;
  if v_invitation.branch_id is not null and not exists (
    select 1 from public.branches b where b.id = v_invitation.branch_id and b.organization_id = v_invitation.organization_id and b.status = 'active'
  ) then raise exception 'organization_branch_not_found'; end if;
  ...
  return query select v_invitation.id, v_invitation.organization_id, o.name, v_membership_id from public.organizations o where o.id = v_invitation.organization_id;
end;
$$;
