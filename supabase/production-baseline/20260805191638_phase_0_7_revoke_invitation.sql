...
begin
  ...
  select i.* into v_invitation from public.organization_invitations i where i.id = p_invitation_id and i.status = 'pending' for update;
  if not found then raise exception 'pending_invitation_not_found'; end if;
  if not private.has_org_permission(v_invitation.organization_id, 'member.invite') then raise exception 'member_invite_permission_required'; end if;
  update public.organization_invitations set status = 'revoked' where id = p_invitation_id and status = 'pending';
  return true;
end;
$$;
