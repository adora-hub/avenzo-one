...
-- The plan-version display column is label, not name.
...
begin
  ...
  select
    ...
    plan.code, v.id, v.label, v.grace_period_days,
...
comment on function public.platform_billing_transfer_fulfillment_queue_v2() is
  'Returns accepted transfer proofs with the current server-side approval requirement. Uses canonical plan code and plan-version label columns.';
