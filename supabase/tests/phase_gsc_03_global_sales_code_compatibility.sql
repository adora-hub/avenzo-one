...
begin
  if not exists (
    ...
    raise exception 'gsc03_legacy_fixture_missing';
  end if;

  if (select sales_code from public.skus
      where id = '00000000-0000-4000-8000-00000000d601') is distinct from 'CF-LEGACY-01' then
    raise exception 'gsc03_legacy_sales_code_changed';
  end if;
  if (select standard_version from public.sales_code_sequences
      where id = '00000000-0000-4000-8000-00000000d701') is distinct from 'legacy' then
  ...
  exception when check_violation then
    if sqlstate <> '23514'
       or sqlerrm not like '%sales_code_legacy_sequence_read_only%' then raise; end if;
  end;
