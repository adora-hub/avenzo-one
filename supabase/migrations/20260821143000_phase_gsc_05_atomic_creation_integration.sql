...
begin
  ...
  -- Fixed GSC-05 command-lock namespace seed. The command UUID is globally
  -- unique by PK, so every tenant contending for the same ID must serialize
  -- before the tenant/payload comparison below. Do not include Organization.
