-- AVENZO ONE Phase 1.1.3.8.5.1: configurable bank-transfer approval policy.
-- This phase stores and audits the policy only. Enforcement begins in Phase 1.1.3.8.5.2.
...
insert into public.billing_transfer_approval_policies (policy_key)
values ('default')
...
create table private.billing_transfer_approval_policy_events (
  ...
  constraint billing_transfer_approval_policy_event_reason_check
    check (length(btrim(reason)) between 10 and 2000)
);

create index billing_transfer_approval_policy_events_created_idx
  on private.billing_transfer_approval_policy_events (created_at desc, id desc);
...
begin
  ...
  return query
  select p.policy_key, p.currency, p.single_admin_limit,
    p.require_two_person_on_risk, p.version, p.updated_by,
    u.email::text, p.updated_at
  from public.billing_transfer_approval_policies p
...
create or replace function public.platform_update_billing_transfer_approval_policy(
  p_single_admin_limit numeric,
  p_require_two_person_on_risk boolean,
  p_reason text,
  p_command_id uuid,
...
begin
  ...
  if p_single_admin_limit is null or p_single_admin_limit < 0 or p_single_admin_limit > 100000000 then
    raise exception 'single_admin_limit_invalid';
  end if;
  ...
  if length(btrim(coalesce(p_reason, ''))) not between 10 and 2000 then
    raise exception 'approval_policy_reason_invalid';
  end if;
  ...
  if v_previous.version <> p_expected_version then
    raise exception 'approval_policy_version_conflict';
  end if;

  update public.billing_transfer_approval_policies
  set single_admin_limit = round(p_single_admin_limit, 2),
      require_two_person_on_risk = p_require_two_person_on_risk,
      version = version + 1,
      updated_by = v_actor,
      updated_at = now()
  where policy_key = 'default'
  ...
  insert into private.billing_transfer_approval_policy_events (
    command_id, previous_policy, new_policy, reason, actor_user_id, actor_email
  ) values (
    p_command_id, to_jsonb(v_previous), to_jsonb(v_result), btrim(p_reason), v_actor, v_actor_email
  );
...
comment on table public.billing_transfer_approval_policies is
  'Singleton policy configuration for bank-transfer approval. Enforcement begins in Phase 1.1.3.8.5.2.';
comment on function public.platform_update_billing_transfer_approval_policy(numeric, boolean, text, uuid, bigint) is
  'AAL2 Super Admin-only, version-checked and idempotent update with immutable audit evidence.';
